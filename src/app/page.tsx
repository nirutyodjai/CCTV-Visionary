    ...
    <div className="flex flex-col min-h-screen bg-background">
      <header className="container mx-auto px-4 sm:px-6 lg:px-8 py-4 flex justify-between items-center sticky top-0 bg-background/80 backdrop-blur-sm z-50">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-xl sm:text-2xl font-bold text-foreground">โปรแกรมวางแผน CCTV</h1>
        </div>
        <nav className="flex items-center gap-4">
           ...
           <Link href="/planner" passHref>
             <Button className="btn-glow">เริ่มต้นใช้งาน</Button>
           </Link>
        </nav>
      </header>
        ...
        <section id="features" className="bg-card/50 py-20">
          <div className="container mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center mb-12">
                <h3 className="text-2xl sm:text-3xl font-bold text-foreground">ชุดเครื่องมือที่ครอบคลุมสำหรับการออกแบบระบบที่ทันสมัย</h3>
                <p className="mt-2 text-lg text-muted-foreground">ทุกสิ่งที่คุณต้องการเพื่อนำโปรเจกต์จากแนวคิดไปสู่การปฏิบัติจริง</p>
            </div>
            <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
              {featureList.map((feature, index) => (
                  <Card key={index} className="bg-card/80 backdrop-blur-sm transform hover:scale-105 transition-transform duration-300 ease-in-out">
        ...
        <section id="how-it-works" className="py-20 bg-background">
            <div className="container mx-auto px-4 sm:px-6 lg:px-8">
                <div className="text-center mb-16">
                    <h3 className="text-2xl sm:text-3xl font-bold text-foreground">จากแนวคิดสู่การปฏิบัติใน 4 ขั้นตอนง่ายๆ</h3>
                    <p className="mt-2 text-lg text-muted-foreground">ขั้นตอนการทำงานที่คล่องตัวเพื่อเร่งกระบวนการออกแบบของคุณ</p>
                </div>
                <div className="relative">
                    <div className="hidden md:block absolute top-8 left-0 w-full h-0.5 bg-border" style={{top: '3rem'}}></div>
                    ...
                    <div className="grid md:grid-cols-4 gap-12 relative">
                        {workflowSteps.map((step) => (
                            <div key={step.title} className="text-center">
                                ...
                                <h4 className="text-xl font-semibold mt-6 text-foreground">{step.title}</h4>
