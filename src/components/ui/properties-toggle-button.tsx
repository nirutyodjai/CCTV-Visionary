...
const PropertiesToggleButton = React.forwardRef<HTMLButtonElement, PropertiesToggleButtonProps>(
    ({ isOpen, className, ...props }, ref) => {
        const ArrowIcon = (
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1024 1024" height="25px" width="25px" className={cn("transition-transform duration-500", !isOpen && 'rotate-180')}>
        ...
        return (
            <button ref={ref} className={cn("bg-card text-card-foreground text-center w-48 rounded-2xl h-14 relative text-xl font-semibold group hidden md:inline-flex items-center justify-center", className)} type="button" {...props}>
